...
class DestImageActionError(StorageException):
    def __init__(self, imgUUID, sdUUID, msg = ""):
        self.value = "image=%s, dest domain=%s: msg=%s" % (imgUUID, sdUUID, msg)
    code = 260
...
class CurrentVersionTooAdvancedError(StorageException):
    def __init__(self, sdUUID, curVer, expVer):
        self.value = ""
        self.message = "Current domain `%s` version is too advanced, expected `%d` and found `%d`" % (sdUUID, expVer, curVer)
    code = 395
...
class TaskAborted(GeneralException):
    ...
    def __str__(self):
        return "%s: %s - code %s" % (self.message, repr(self.value), self.abortedcode)
...
class LogicalVolumeExtendError(StorageException):
    def __init__(self, vgname, lvname, newsize):
        self.value = "vgname=%s lvname=%s newsize=%s" % (vgname, lvname, newsize)
    code = 554
...
class SpmParamsMismatch(StorageException):
    def __init__(self, oldlver, oldid, prevLVER, prevID):
        self.value = "expected previd:%s lver:%s got request for previd:%s lver:%s" % (oldid, oldlver, prevID, prevLVER)
    code = 652
...
class MetaDataSealIsBroken(MetaDataGeneralError):
    def __init__(self, cksum, computed_cksum):
        self.value = "cksum = %s, computed_cksum = %s" % (cksum, computed_cksum)
    code = 752
