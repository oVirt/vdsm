...
def networks_caps(running_config):
    ...
    def get_engine_expected_top_dev(net, attrs):
        ...
        vlan = attrs.get('vlan')
        return (net if tobool(attrs.get('bridged', True))
    ...
    for network, attrs in iter_ovs_nets(running_config.networks):
        ...
        net_info = _get_net_info(attrs, actual_top_dev, dhcpv4ifaces,
                                 dhcpv6ifaces, routes)
...
def _update_expected_ip_info(caps, running_config):
    ...
    def copy_net_info(source, destination):
        KEYS = {'addr', 'gateway', 'netmask', 'dhcpv4', 'ipv4addrs',
                'ipv6addrs', 'ipv6gateway', 'dhcpv6', 'cfg'}
        for key in KEYS:
            destination[key] = source[key]

    for network, attrs in iter_ovs_nets(running_config.networks):
        if not tobool(attrs.get('bridged', True)) and 'vlan' not in attrs:
            bond = attrs.get('bond')
            nic = attrs.get('nic')
            if bond is not None:
                copy_net_info(
