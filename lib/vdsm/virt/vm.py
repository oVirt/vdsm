...
class Vm(object):
    ...
    def __init__(self, cif, params, recover=False):
        ...
        if self._src_domain_xml is not None:
            if self._altered_state.from_snapshot:
                self._src_domain_xml = self._correct_disk_volumes_from_xml(
        ...
        self._creationThread = concurrent.thread(
            self._startUnderlyingVm, name="vm/" + self.id[:8]
        )
        ...
        if (
            ...
            (
                params.get('status') == vmstatus.MIGRATION_SOURCE
                or
                # no status from recovery file available (new style)
                params.get('status') is None
                and self._recovering_migration()
            )
        ):
    ...
    def monitorable(self):
        if (
            self._altered_state
            or self.post_copy != migration.PostCopyPhase.NONE
        ):
            return False
    ...
    def hugepagesz(self):
        ...
        if hugepagesz not in hugepages.supported():
            ...
            self.log.warning(
                'Invalid hugepage size configured, '
    ...
    def _init_from_metadata(self):
        ...
        with self._md_desc.values() as md:
            self._destroy_on_reboot = (
                md.get('destroy_on_reboot', False)
                or self._domain.on_reboot_config() == 'destroy'
            )
            ...
            self._resume_behavior = md.get(
                'resumeBehavior', ResumeBehavior.AUTO_RESUME
            )
    ...
    def min_cluster_version(self, major, minor):
        ...
        return (
            cluster_major > major
            or cluster_major == major
            and cluster_minor >= minor
        )

    def _init_external_data(
        self, kind, initial_data, check_function, read_function, write_function
    ):
        """
        ...
        if initial_data is None:
            ...
            if self.lastStatus != vmstatus.MIGRATION_DESTINATION:
                self.log.info(
                    "Device present but no data provided for %s", kind
                )
            engine_hash = ''
        else:
            ...
            try:
                ...
                self.log.error(
                    "Failed to initialize external data %s: %s", kind, e
                )
    ...
    def set_last_status(self, value, check_last_status=None):
        with self._statusLock:
            if (
                check_last_status is not None
                and self._lastStatus != check_last_status
            ):
            ...
            if self._lastStatus == vmstatus.DOWN:
                self.log.warning(
                    'trying to set state to %s when already Down', value
                )
    ...
    def _normalizeVdsmImg(self, drv):
        ...
        if drv['device'] == 'disk':
            volsize = self.getVolumeSize(
                drv['domainID'], drv['poolID'], drv['imageID'], drv['volumeID']
            )
    ...
    def _dev_spec_update_with_vm_conf(self, dev):
        ...
        if dev['type'] in (hwclass.DISK, hwclass.NIC):
            ...
            self.log.debug(
                'device %s: adding VM custom properties %s',
                dev['type'],
                vm_custom,
            )
    ...
    def updateDriveIndex(self, drv):
        drv['index'] = self.__getNextIndex(
            self._usedIndices[self._indiceForIface(drv['iface'])]
        )
    ...
    def _mem_subtract_nvdimms(self, mem_size_mb):
        ...
        for nvdimm in self._domain.get_device_elements_with_attrs(
            ...
            try:
                ...
                self.log.exception(
                    'Failed to parse NVDIMM size: %r', size_element.text
                )
                continue
            nvdimms += size
    ...
    def _startUnderlyingVm(self):
        ...
        try:
            ...
            if (
                self.recovering
                and self._lastStatus == vmstatus.WAIT_FOR_LAUNCH
            ):
                if self._exit_info:
                    self.set_last_status(
                        vmstatus.DOWN, vmstatus.WAIT_FOR_LAUNCH
                    )
                else:
    ...
    def _recover_status(self):
        ...
        if state == libvirt.VIR_DOMAIN_PAUSED:
            if reason == libvirt.VIR_DOMAIN_PAUSED_POSTCOPY:
                self.set_last_status(
                    vmstatus.MIGRATION_SOURCE, vmstatus.WAIT_FOR_LAUNCH
                )
            ...
            self.log.error(
                "Unexpected VM state: %s (reason %s)", state, reason
            )
    ...
    def refresh_destination_volume(self, volInfo):
        ...
        if vol_size.apparentsize < volInfo['newSize']:
            raise exception.CannotRefreshDisk(
                ...
                "size {} < expected size {}".format(
                    vol_size.apparentsize, volInfo["newSize"]
                )
            )
    ...
    def extend_volume_completed(self):
        ...
        try:
            ...
            if (
                current_status == vmstatus.UP
                and e.get_error_domain() == libvirt.VIR_FROM_QEMU
                and e.get_error_code() == libvirt.VIR_ERR_OPERATION_INVALID
            ):
    ...
    def cont(
        self,
        afterState=vmstatus.UP,
        guestCpuLocked=False,
        ignoreStatus=False,
        guestTimeSync=False,
    ):
        """
        ...
        try:
            ...
            self._setGuestCpuRunning(
                self.isDomainRunning(), guestCpuLocked=True
            )
            self._logGuestCpuStatus('continue')
            self._lastStatus = afterState
            self._pause_code = None
            self._pause_time = None
            if guestTimeSync or config.getboolean(
                'vars', 'time_sync_cont_enable'
            ):
                self.syncGuestTime()
        finally:
            if not guestCpuLocked:
    ...
    def shutdown(self, delay, message, reboot, timeout, force):
        ...
        if reboot:
            self._guestEvent = vmstatus.REBOOT_IN_PROGRESS
            powerDown = VmReboot(
                self, delay, message, timeout, force, self._powerDownEvent
            )
        else:
            self._guestEvent = vmstatus.POWERING_DOWN
            powerDown = VmShutdown(
                self, delay, message, timeout, force, self._powerDownEvent
            )
        ...
        with self._volPrepareLock:
            for drive in drives:
                try:
                    ...
                    self.log.warning(
                        "Drive transient volume deletion failed "
                        "for drive %s",
                        drive,
                        exc_info=True,
                    )
                ...
                try:
                    ...
                    self.log.exception("Drive teardown failure for %s", drive)
    ...
    def setDownStatus(self, code, exitReasonCode, exitMessage=''):
        if not exitMessage:
            exitMessage = vmexitreason.exitReasons.get(
                exitReasonCode, 'VM terminated'
            )
        exit_info = {
            'exitCode': code,
        ...
        try:
            ...
            self.log.info(
                "Changed state to Down: %s (code=%i)",
                exitMessage,
                exitReasonCode,
            )
            try:
            ...
            if self.isMigrating() and self.post_copy:
                ...
                if 'memoryStats' in stats:
                    ...
                    if oga_stats['memUsage'] == '0':
                        ...
                        oga_stats['memUsage'] = str(
                            int(
                                100
                                - float(
                                    int(stats['memoryStats']['mem_free'])
                                    / 1024
                                )
                                / self.mem_size_mb()
                                * 100
    ...
    def _getDownVmStats(self):
        stats = {'vmId': self.id, 'status': self.lastStatus}
    ...
    def _getRunningVmStats(self):
        ...
        if devices_hash is not None:
            stats['hash'] = str(
                hash((devices_hash, self.guestAgent.diskMappingHash))
            )
        for kind, attribute in [
            ...
            if kind in self._external_data:
                stats[attribute] = self._external_data[kind].data.engine_hash
    ...
    def _getVmStatus(self):
        def _getVmStatusFromGuest():
            ...
            self.log.debug(
                ...
                self._guestEvent,
                self._guestEventTime,
                now,
                now - self._guestEventTime,
            )
            if (
                now - self._guestEventTime < 5 * GUEST_WAIT_TIMEOUT
                and self._guestEvent == vmstatus.POWERING_DOWN
            ):
                return self._guestEvent
            if (
                self.guestAgent
                and (
                    self.guestAgent.isResponsive()
                    or self.cif.qga_poller.is_active(self.id)
                )
                and self.guestAgent.getStatus()
            ):
                agent_status = self.guestAgent.getStatus()
                if (
                    self._guestEvent == vmstatus.POWERING_UP
                    and agent_status == vmstatus.UP
                ):
        ...
        if (
            ...
            and self.post_copy == migration.PostCopyPhase.RUNNING
        ):
    ...
    def _getGuestStats(self):
        ...
        if realMemUsage != 0:
            memUsage = 100 - float(realMemUsage) / self.mem_size_mb() * 100
        else:
            memUsage = 0
    ...
    def migrate(self, params):
        ...
        try:
            ...
            if self._lastStatus in (vmstatus.WAIT_FOR_LAUNCH, vmstatus.DOWN):
                raise exception.NoSuchVM()
    ...
    def onJobCompleted(self, args):
        if (
            not self._migrationSourceThread.started
            and not self._migrationSourceThread.recovery
        ) or self._migrationSourceThread.hibernating:
            return
        stats = args[0]
    ...
    def _finish_migration_recovery(self):
        ...
        if (
            state == libvirt.VIR_DOMAIN_SHUTOFF
            and reason == libvirt.VIR_DOMAIN_SHUTOFF_MIGRATED
        ):
            self.setDownStatus(NORMAL, vmexitreason.MIGRATION_SUCCEEDED)
            if self.post_copy == migration.PostCopyPhase.RUNNING:
    ...
    def migrateCancel(self):
        ...
        try:
            ...
            self._migrationSourceThread.status['status'][
                'message'
    ...
    def switch_migration_to_post_copy(self):
        ...
        try:
            ...
            try:
                ...
                self.log.exception(
                    "Failed to start guest agent after "
    ...
    def _teardown_devices(self, devices=None):
        ...
        for device in devices:
            try:
                ...
                self.log.exception(
                    'Failed to tear down device %s, device in '
                    'inconsistent state',
                    device.device,
                )
    ...
    def _undefine_domain(self):
        if self._external:
            ...
            self.log.info("Will not undefine external VM %s", self.id)
            return
        ...
        try:
            ...
            self.log.warning(
                "Failed to undefine VM '%s' (error=%i)",
                self.id,
                e.get_error_code(),
            )
        except virdomain.NotConnectedError:
    ...
    def _vmDependentInit(self):
        ...
        if self.lastStatus == vmstatus.RESTORING_STATE:
            try:
                ...
                self.log.exception(
                    "Unexpected error on guest after "
        ...
        if self.lastStatus not in (
            ...
            try:
                ...
                if self.recovering:
                    self.log.warning(
                        "Couldn't retrieve initial pause code "
                        "from libvirt: %s",
                        e,
                    )
                else:
    ...
    def _setup_devices(self):
        ...
        for dev_object in self._tracked_devices():
            try:
                ...
                self.log.exception(
                    "Failed to setup device %s", dev_object.device
                )
    ...
    def _override_disk_device_config(self, disk_params):
        ...
        self.log.debug(
            "Overridden %d legacy drive configurations", len(disk_params)
        )
    ...
    def _run(self):
        ...
        if (
            not self.recovering
            and self._altered_state.origin != _MIGRATION_ORIGIN
        ):
    ...
    def _correct_disk_volumes_from_xml(self, srcDomXML, engine_xml):
        ...
        """
        domain = MutableDomainDescriptor(srcDomXML)
        engine_domain = DomainDescriptor(
            engine_xml, xml_source=XmlSource.INITIAL
        )
    ...
    def _remove_backingstore_configuration(self, element):
        ...
        try:
            vmxml.remove_child(
                element, vmxml.find_first(element, 'backingStore')
            )
    ...
    def hotplugNic(self, params):
        ...
        # In any case we need below rollback for all kind of failures.
            ...
            self.log.exception(
                "setPortMirroring for network %s failed", network
            )
            ...
            self.hotunplugNic(
                hotunplug_params, port_mirroring=mirroredNetworks
            )
            return response.error('hotplugNic', str(e))
    ...
    def _hotunplug_device(
        self, device_xml, device, device_hwclass, update_metadata=False
    ):
        try:
    ...
    def hotplugHostdev(self, device_xmls):
        for xml in device_xmls:
            ...
            try:
                ...
                self.log.exception(
                    'Could not detach a device from a host: %s', xml
                )
    ...
    def _lookupDeviceByPath(self, path):
        for dev in self._devices[hwclass.DISK][:]:
            try:
                ...
                continue
        raise LookupError(
            'Device instance for device with path {0} not found'
            ''.format(path)
        )
    ...
    def _updateInterfaceDevice(self, params):
        try:
            ...
            linkValue = (
                'up'
                if conv.tobool(params.get('linkActive', netDev.linkActive))
                else 'down'
            )
            ...
            UpdatePortMirroringError,
        ) as e:
            raise exception.UpdateDeviceFailed(str(e))

    @contextmanager
    def setLinkAndNetwork(
        self,
        dev,
        linkValue,
        networkValue,
        ...
        vmdevices.network.update_filterref_xml(
            vnicXML, filter, filterParameters
        )
        ...
        try:
            try:
                ...
                self._dom.updateDeviceFlags(
                    vnicStrXML, libvirt.VIR_DOMAIN_AFFECT_LIVE
                )
            ...
            self.log.warning(
                'Rolling back link and net for: %s', dev.alias, exc_info=True
            )
            self._dom.updateDeviceFlags(
    ...
    def updatePortMirroring(self, nic, networks):
        devName = nic.name
        netsToDrop = [net for net in nic.portMirroring if net not in networks]
        netsToAdd = [net for net in networks if net not in nic.portMirroring]
        mirroredNetworks = []
        droppedNetworks = []
        try:
            ...
            self.log.exception(
                ...
                (
                    'setPortMirroring'
                    if network in netsToAdd
                    else 'unsetPortMirroring'
                ),
                network,
            )
    ...
    def hotunplugNic(self, params, port_mirroring=None):
        ...
        if nic:
            ...
            hooks.before_nic_hotunplug(nicXml, self._custom, params=nic.custom)
            # TODO: this is debug information. For 3.6.x we still need to
            # see the XML even with 'info' as default level.
            self.log.info("Hotunplug NIC xml: %s", nicXml)
        else:
            self.log.error(
                "Hotunplug NIC failed - NIC not found: %s", nicParams
            )
            return response.error('hotunplugNic', "NIC not found")

        try:
            self._hotunplug_device(
                nicXml, nic, hwclass.NIC, update_metadata=True
            )
        except (HotunplugTimeout, libvirt.libvirtError) as e:
            hooks.after_nic_hotunplug_fail(
                nicXml, self._custom, params=nic.custom
            )
            return response.error('hotunplugNic', str(e))

        hooks.after_nic_hotunplug(nicXml, self._custom, params=nic.custom)
    ...
    def _assignCpusets(self, cpusets):
        ...
        for vcpu, cpuset in enumerate(cpusets):
            ...
            try:
                self.pin_vcpu(
                    vcpu,
                    cpumanagement.libvirt_cpuset_spec(
    ...
    def setNumberOfCpus(self, numberOfCpus, cpusets=None):
        ...
        try:
            self._dom.setVcpusFlags(
                numberOfCpus, libvirt.VIR_DOMAIN_AFFECT_CURRENT
            )
        except libvirt.libvirtError as e:
    ...
    def updateVmPolicy(self, params):
        ...
        if not params:
            ...
            return response.error(
                'MissParam', 'updateVmPolicy got an empty policy.'
            )
        ...
        if params:
            self.log.warning(
                "updateVmPolicy got unknown parameters: %s",
                ", ".join(params.keys()),
            )
    ...
    def _getVmPolicy(self):
        ...
        return vmxml.find_first(
            metadata, xmlconstants.METADATA_VM_TUNE_ELEMENT, None
        )
    ...
    def find_device_by_name_or_path(self, device_name=None, device_path=None):
        ...
        raise LookupError(
            "No such disk {} with path {}".format(device_name, device_path)
        )
            ...
            try:
                ...
                if not ioTune:
                    ...
                    res = self._dom.blockIoTune(
                        device.name, libvirt.VIR_DOMAIN_AFFECT_LIVE
                    )
    ...
    def setIoTune(self, tunables):
        for io_tune_change in tunables:
            ...
            try:
                self._dom.setBlockIoTune(
                    found_device.name, io_tune, libvirt.VIR_DOMAIN_AFFECT_LIVE
                )
            except libvirt.libvirtError as e:
            ...
            # TODO: this is debug information. For 3.6.x we still need to
            # see the XML even with 'info' as default level.
            self.log.info("New device XML for %s: %s", found_device.name, xml)
    ...
    def _createTransientDisk(self, diskParams):
        ...
        driveFormat = (
            ...
            if diskParams['format'] == 'cow'
            else qemuimg.FORMAT.RAW
        )
    ...
    def hotplugDisk(self, params):
        ...
        driveXml = hooks.before_disk_hotplug(
    ...
    def hotunplugDisk(self, params):
        ...
        if drive is None:
            ...
            try:
                ...
                self.log.error(
                    "Hotunplug disk failed - Disk not found: %s", diskParams
                )
        ...
        try:
            self._hotunplug_device(
                driveXml, drive, hwclass.DISK, update_metadata=True
            )
        except HotunplugTimeout as e:
    ...
    def hotunplugLease(self, params):
        try:
            ...
            raise exception.HotunplugLeaseFailed(
                reason="No such lease", lease=params
            )
    ...
    def _waitForDeviceRemoval(self, device):
        ...
        with utils.stopwatch("Hotunplug %r" % device):
            deadline = vdsm.common.time.monotonic_time() + config.getfloat(
                'vars', 'hotunplug_timeout'
            )
    ...
    def paused_on_io_error(self):
        ...
        return (
            ...
            and reason == libvirt.VIR_DOMAIN_PAUSED_IOERROR
        )
                ...
                if state[0] == libvirt.VIR_DOMAIN_PAUSED:
                    if state[1] == libvirt.VIR_DOMAIN_PAUSED_MIGRATION:
                        raise MigrationError(
                            "Migration Error - Timed out "
                            "(did not receive success "
                            "event)"
                        )
                self.log.debug(
                    "NOTE: incoming_migration_vm_running event has "
    ...
    def findDriveByUUIDs(self, drive):
        ...
        if "domainID" in drive:
            tgetDrv = (drive["domainID"], drive["imageID"], drive["volumeID"])
    ...
    def updateDriveVolume(self, vmDrive):
        ...
        volSize = self.getVolumeSize(
            vmDrive.domainID, vmDrive.poolID, vmDrive.imageID, vmDrive.volumeID
        )
    ...
    def updateDriveParameters(self, driveParams):
        ...
        for vmDrive in self._devices[hwclass.DISK][:]:
            ...
            self.log.error(
                "Unable to update the drive object for: %s",
                driveParams["name"],
            )
    ...
    def query_drive_volume_index(self, drive, vol_id):
        ...
        if actual_chain is None:
            raise RuntimeError(
                "Cannot get drive {} alias {} actual volume chain".format(
    ...
    def dump_checkpoint(self, checkpoint_id):
        dom = backup.DomainAdapter(self)
        return backup.dump_checkpoint(dom, checkpoint_id=checkpoint_id)
    ...
    def diskReplicateStart(self, srcDisk, dstDisk, need_extend=True):
        ...
        try:
            ...
            self.log.error(
                "Unable to set the replication for disk '%s' with "
                "destination '%s'",
                drive.name,
                replica,
            )
        ...
        try:
            ...
            self.log.exception(
                "Unable to start replication for %s to %s", drive.name, replica
            )
        ...
        if need_extend and (drive.chunked or drive.replicaChunked):
            try:
                ...
                self.log.exception(
                    "Initial extension request failed for %s", drive.name
                )
    ...
    def diskReplicateFinish(self, srcDisk, dstDisk):
        ...
        if drive.hasVolumeLeases:
            self.log.error(
                "Drive has volume leases, replication not "
        ...
        if drive.transientDisk:
            self.log.error(
                "Transient disk, replication not supported "
                "(drive: %r, srcDisk: %r)",
                drive.name,
                srcDisk,
            )
        ...
        if not drive.isDiskReplicationInProgress():
            raise exception.ReplicationNotInProgress(
                vmId=self.id, driveName=drive.name, srcDisk=srcDisk
            )
        ...
        if (
            not isinstance(blkJobInfo, dict)
            or 'cur' not in blkJobInfo
            or 'end' not in blkJobInfo
        ):
            self.log.error(
                "Replication job not found (drive: %r, "
                "srcDisk: %r, job: %r)",
                drive.name,
                srcDisk,
                blkJobInfo,
            ...
            try:
                ...
                self.log.exception(
                    "Failed to teardown replica of %s: %s", drive, e
                )
        ...
        if blkJobInfo['cur'] != blkJobInfo['end']:
            self.log.error(
                "Replication job unfinished (drive: %r, "
                "srcDisk: %r, job: %r)",
                drive.name,
                srcDisk,
                blkJobInfo,
        ...
        dstDiskCopy.update(
            {'device': drive.device, 'name': drive.name, 'type': drive.type}
        )
        ...
        if srcDisk != dstDisk:
            self.log.debug(
                "Stopping the disk replication switching to the "
                "destination drive: %s",
                dstDisk,
            )
            ...
            self.log.debug(
                "Stopping the disk replication remaining on the "
                "source drive: %s",
                dstDisk,
            )
            blockJobFlags = 0
    ...
    def _setDiskReplica(self, drive, replica):
        ...
        if drive.isDiskReplicationInProgress():
            raise RuntimeError(
                "Disk '%s' already has an ongoing " "replication" % drive.name
            )
    ...
    def _updateDiskReplica(self, drive):
        ...
        if not drive.isDiskReplicationInProgress():
            raise RuntimeError(
                "Disk '%s' does not have an ongoing "
    ...
    def _diskSizeExtendCow(self, drive, newSizeBytes):
        try:
            ...
            self.log.exception(
                "Cannot get disk %s current size: %s", drive.name, e
            )
        ...
        self._setVolumeSize(
            drive.domainID, drive.poolID, drive.imageID, drive.volumeID, 0
        )

        flags = libvirt.VIR_DOMAIN_BLOCK_RESIZE_BYTES
        self.log.debug(
            "Calling blockResise drive=%s size=%s flags=%s",
            drive.name,
            newSizeBytes,
            flags,
        )
    ...
    def _getLUNDevice(self, drive, refresh=True):
        ...
        if res['status']['code'] != 0:
            raise RuntimeError(
                "Cannot find device {} for drive {}: {}".format(
                    drive.GUID, drive, res["status"]["message"]
                )
            )
        ...
        for dev in self._devices[hwclass.DISK]:
            if dev.device == "cdrom":
                try:
                    ...
                    self.log.error("Failed to recover CD %s: %s", dev.name, e)
    ...
    def _recover_loading_cd(self, block_dev, current, change):
        ...
        if changed:
            ...
            self.log.info("Applying CD change to metadata, change=%s", change)
    ...
    def _changeBlockDev(
        self, vmDev, blockdev, drivespec, iface=None, force=True
    ):
        try:
        ...
        if drive_spec:
            try:
                ...
                raise exception.CannotPrepareImage(
                    reason=str(e), drive_spec=drive_spec
                )
        else:
    ...
    def _check_fips_params_valid(self, params):
        ...
        if fips and params.get('vncUsername') is None:
            raise exception.GeneralException('FIPS mode requires vncUsername')

    def _setTicketForGraphicDev(
        self,
        graphics,
        otp,
        seconds,
        ...
        try:
            ...
            self._consoleDisconnectAction = (
                disconnectAction or ConsoleDisconnectAction.LOCK_SCREEN
            )
    ...
    def _findGraphicsDeviceXMLByType(self, deviceType):
        ...
        try:
            return next(
                dom_desc.get_device_elements_with_attrs(
    ...
    def onIOError(self, blockDevAlias, err, action):
        ...
        if action == libvirt.VIR_DOMAIN_EVENT_IO_ERROR_PAUSE:
            self.log.info(
                'abnormal vm stop device %s error %s', blockDevAlias, err
            )
            ...
            self.log.info(
                'I/O error %s device %s reported to guest OS',
                reason,
                blockDevAlias,
            )
        else:
            ...
            self.log.warning(
                'unexpected action %i on device %s error %s',
                action,
                blockDevAlias,
                reason,
            )
    ...
    def hasSpice(self):
        return bool(
            list(
                self._domain.get_device_elements_with_attrs(
    ...
    def _updateDomainDescriptor(self, xml=None):
        ...
        self._domain = DomainDescriptor(
            domxml,
            xml_source=(
                XmlSource.INITIAL if xml is not None else XmlSource.LIBVIRT
            ),
        )
        if xml is None:
            for name, _, state in self._domain.all_channels():
                if (
                    name == vmchannels.QEMU_GA_DEVICE_NAME
    ...
    def _update_metadata(self):
        with self._md_desc.values() as vm:
            ...
            try:
                if (
                    not self._snapshot_job
                    or not jobs.get(self._snapshot_job['jobUUID']).active
                ):
                    try:
                        del vm['snapshot_job']
                    except KeyError:
    ...
    def releaseVm(self, gracefulAttempts=1):
        ...
        with self._releaseLock:
            ...
            for nic in self._devices[hwclass.NIC]:
                if hasattr(nic, 'name'):
                    for network in nic.portMirroring[:]:
                        supervdsm.getProxy().unsetPortMirroring(
                            network, nic.name
                        )
    ...
    def _destroyVmGraceful(self):
        ...
        try:
            ...
            if (
                self.lastStatus == vmstatus.DOWN
                and e.get_error_code() == libvirt.VIR_ERR_NO_DOMAIN
            ):
    ...
    def _deleteVm(self):
        ...
        try:
            ...
            self.log.debug(
                "Total desktops after destroy of %s is %d",
                self.id,
                len(self.cif.vmContainer),
            )

    def destroy(self, gracefulAttempts=1, reason=vmexitreason.ADMIN_SHUTDOWN):
    ...
    def doDestroy(
        self, gracefulAttempts=1, reason=vmexitreason.ADMIN_SHUTDOWN
    ):
        for dev in self._customDevices():
            hooks.before_device_destroy(
                dev._deviceXML, self._custom, dev.custom
            )
    ...
    def qemuGuestAgentShutdown(self):
        ...
        try:
            ...
            self.log.warning(
                'failed to invoke qemuGuestAgentShutdown: '
    ...
    def qemuGuestAgentReboot(self):
        try:
            ...
            # so ignore error and quickly exit
            self.log.warning(
                'failed to invoke qemuGuestAgentReboot: '
                'domain not connected'
            )
            raise exception.VMIsDown()
        except libvirt.libvirtError as e:
            ...
            if is_active_check:
                ...
                self.log.debug("Reboot by QEMU Guest Agent failed", exc_info=e)
            raise exception.NonResponsiveGuestAgent()
    ...
    def setBalloonTarget(self, target):
        ...
        if not self._ballooning_enabled:
            self.log.debug(
                'Ignoring balloon target change for VM with'
                ' disabled ballooning'
            )
            return
    ...
    def setCpuTuneQuota(self, quota):
        try:
            ...
            return response.error(
                'cpuTuneErr', 'an integer is required for period'
            )
        except libvirt.libvirtError as e:
    ...
    def _handle_libvirt_domain_stopped(self, detail):
        if (
            ...
            and self.lastStatus == vmstatus.MIGRATION_SOURCE
        ):
            try:
                hooks.after_vm_migrate_source(self._domain.xml, self._custom)
                for dev in self._customDevices():
            ...
            detail == libvirt.VIR_DOMAIN_EVENT_STOPPED_SAVED
            and self.lastStatus == vmstatus.SAVING_STATE
        ):
    ...
    def _handle_libvirt_domain_suspended(self, detail):
        ...
        if (
            ...
            and detail == libvirt.VIR_DOMAIN_EVENT_SUSPENDED_PAUSED
        ):
    ...
    def _handle_libvirt_domain_resumed(self, detail):
        ...
        if detail == libvirt.VIR_DOMAIN_EVENT_RESUMED_UNPAUSED:
            ...
            self.lastStatus == vmstatus.MIGRATION_DESTINATION
            and detail == libvirt.VIR_DOMAIN_EVENT_RESUMED_POSTCOPY
        ):
            ...
            if self._shutdownReason is None:
                if detail == libvirt.VIR_DOMAIN_EVENT_SHUTDOWN_HOST:
                    ...
                    self.log.warning(
                        ...
                        " libvirt: %s. Assuming user shutdown.",
                        detail,
                    )
                    self._shutdownReason = vmexitreason.USER_SHUTDOWN
                    host_shutdown_check_needed = True
        ...
        for devType in self._devices:
            for dev in self._devices[devType]:
                if hasattr(dev, 'alias'):
                    ...
                    self.log.error(
                        "Alias not found for device type %s "
                        "during migration at destination host" % devType
                    )
    ...
    def on_block_job_event(self, drive, job_type, job_status):
        ...
        if job_status == libvirt.VIR_DOMAIN_BLOCK_JOB_COMPLETED:
            self.log.info(
                "Block job %s type %s for drive %s has completed",
                job_id,
                type_name,
                drive,
            )
            ...
            self.log.error(
                "Block job %s type %s for drive %s has failed",
                job_id,
                type_name,
                drive,
            )
            ...
            self.log.error(
                "Block job %s type %s for drive %s was canceled",
                job_id,
                type_name,
                drive,
            )
            ...
            self.log.info(
                "Block job %s type %s for drive %s is ready",
                job_id,
                type_name,
                drive,
            )
    ...
    def sync_volume_chain(self, drive):
        if not isVdsmImage(drive):
            self.log.debug(
                "Skipping drive '%s' which is not a vdsm image", drive.name
            )
            return
        ...
        volumeID = volumes[-1]
        res = self.cif.irs.getVolumeInfo(
            drive.domainID, drive.poolID, drive.imageID, volumeID
        )
        if res['status']['code'] != 0:
            self.log.error(
    ...
    def onDeviceRemoved(self, device_alias):
        ...
        try:
            ...
            try:
                ...
                # such a case.
                self.log.warning(
                    "Removed device not found in devices: %s", device_alias
                )
                self._updateDomainDescriptor()
                return
            else:
    ...
    def prune_bitmaps(self, domainID, imageID, topID, baseID):
        ...
        res = self.cif.irs.prune_bitmaps(
            sdUUID=domainID, imgUUID=imageID, volUUID=topID, baseUUID=baseID
        )
    ...
    def measure(
        self, domainID, imageID, topID, dest_format, backing=True, baseID=None
    ):
        """
    ...
    def _setVolumeSize(self, domainID, poolID, imageID, volumeID, size):
        res = self.cif.irs.setVolumeSize(
            domainID, poolID, imageID, volumeID, size
        )
        if res['status']['code'] != 0:
            raise errors.StorageUnavailableError(
    ...
    @contextmanager
    def qga_context(
        self, timeout=libvirt.VIR_DOMAIN_AGENT_RESPONSE_TIMEOUT_BLOCK
    ):
        """
...
def sync_drive_conf(dev_conf, drive):
    ...
    update_active_path(dev_conf['volumeChain'], drive.volumeID, drive.path)
