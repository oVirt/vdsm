...
def _v3_reset_meta_volsize(vol):
    ...
    if vol.getFormat() == sc.COW_FORMAT:
        qemuVolInfo = qemuimg.info(vol.getVolumePath(), qemuimg.FORMAT.QCOW2)
    ...
    if meta_vol_size != virtual_vol_size:
        log.warning(
            "Fixing the mismatch between the metadata volume size "
...
def v3DomainConverter(repoPath, hostId, domain, isMsd):
    ...
    if domain.getStorageType() in sd.BLOCK_DOMAIN_TYPES:
        if currentVersion != 2:
            log.debug(
                "Unsupported conversion from version %s to version %s",
                currentVersion,
                targetVersion,
            )
    ...
    def v3ReallocateMetadataSlot(domain, allVolumes):
        if domain.getStorageType() not in sd.BLOCK_DOMAIN_TYPES:
            log.debug(
                "The metadata reallocation check is not needed for "
                "domain %s",
                domain.sdUUID,
            )
            return
        ...
        log.debug(
            "Starting metadata reallocation check for domain %s with "
            "metaMaxSlot %s (leases volume size %s)",
            domain.sdUUID,
            metaMaxSlot,
            leasesSize,
        )
        ...
        for volUUID, (imgUUIDs, parentUUID) in allVolumes.items():
            ...
            log.debug(
                "Reallocating metadata slot %s for volume %s",
                metaSlot,
                vol.volUUID,
            )
            ...
            with domain.acquireVolumeMetadataSlot(vol.volUUID) as newMetaSlot:
                ...
                log.debug(
                    "Copying metadata for volume %s to the new slot %s",
                    vol.volUUID,
                    newMetaSlot,
                )
                ...
                log.debug(
                    "Switching the metadata slot for volume %s to %s",
                    vol.volUUID,
                    newMetaSlot,
                )
    ...
    try:
        ...
        for imgUUID in allImages:
            ...
            try:
                try:
                    ...
                    log.error(
                        "Image %s can't be activated.", imgUUID, exc_info=True
                    )
                ...
                for volUUID in imgVolumes:
                    try:
                        ...
                        log.error(
                            "It is not possible to read the volume %s "
                ...
                log.error(
                    "It is not possible to prepare the image %s, the "
                    "volume chain looks damaged",
                    imgUUID,
                    exc_info=True,
                )
                ...
                try:
                    ...
                    log.warning(
                        "Unable to teardown the image %s, this error "
        ...
        log.debug(
            "Finalizing the storage domain upgrade from version %s to "
            "version %s for domain %s",
            currentVersion,
            targetVersion,
            domain.sdUUID,
        )
        ...
        if isMsd:
            try:
                log.error(
                    "Releasing the cluster lock for domain %s with "
                    "host id: %s",
                    domain.sdUUID,
                    hostId,
                )
                ...
                log.error(
                    "Unable to release the cluster lock for domain "
                    "%s with host id: %s",
                    domain.sdUUID,
                    hostId,
                    exc_info=True,
        ...
        try:
            log.error(
                "Releasing the host id %s for domain %s", hostId, domain.sdUUID
            )
            newClusterLock.releaseHostId(hostId, wait=True, unused=True)
        except:
            log.error(
                "Unable to release the host id %s for domain %s",
                hostId,
                domain.sdUUID,
                exc_info=True,
            )
...
def v4DomainConverter(repoPath, hostId, domain, isMsd):
    ...
    log.debug(
        "Starting conversion of domain %s to version %s",
        domain.sdUUID,
        targetVersion,
    )
    ...
    log.debug(
        "Conversion of domain %s to version = %s has been completed.",
        domain.sdUUID,
        targetVersion,
    )
