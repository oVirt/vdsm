...
NvmeofConnectionParameters = namedtuple(
    "NvmeofConnectionParameters",
    "id, nqn, transport, traddr, " "trsvcid, host_nqn, dhchap_key",
)
...
class NvmeofConnection(Connection):
    ...
    def translate_error(cls, e):
        if isinstance(e, nvme.NvmeAuthenticationError):
            return se.NVMeAuthError.code, se.NVMeAuthError.msg
        if isinstance(e, nvme.NvmeConnectionError):
            return se.NVMeConnectionError.code, se.NVMeConnectionError.msg
        if isinstance(e, nvme.NvmeDisconnectionError):
            return se.NVMeDisconnectError.code, se.NVMeDisconnectError.msg
        return super().translate_error(e)
    ...
    def __eq__(self, other):
        ...
        return (
            self._id == other._id
            and self._nqn == other._nqn
            and self._traddr == other._traddr
            and self._trsvcid == other._trsvcid
