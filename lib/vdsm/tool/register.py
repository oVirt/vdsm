...
class Register(object):
    ...
    def __init__(
        ...
        if engine_https_port is None:
            ...
            self.engine_url = "https://{e}:{p}".format(
    ...
    def handshake(self):
        ...
        __GET_VERSION = "https://{e}{u}{c}".format(
            e=self.engine_fqdn, u=ucmd, c="get-version"
        )
        ...
        self.url_CA = "{e}{uc}{c}".format(
            e=self.engine_url, uc=ucmd, c="get-pki-trust"
        )

        self.url_ssh_key = "{e}{uc}{c}".format(
            e=self.engine_url, uc=ucmd, c="get-ssh-trust"
        )

        ureg = (
            "{uc}register&sshUser={sshu}&"
            "sshPort={sshp}&port={mp}".format(
                uc=ucmd,
                sshu=self.ssh_user,
                sshp=self.ssh_port,
                mp=self.vdsm_port,
    ...
    def download_ca(self):
        ...
        if os.path.exists(self.ca_engine):
            ...
            res = self._execute_http_request(
                self.url_CA, cert_validation=False
            )

            with tempfile.NamedTemporaryFile(
                dir=os.path.dirname(self.ca_dir), delete=False
            ) as f:
                f.write(res)
        ...
        if self.fprint and self.fprint.lower() != calculated_fprint.lower():
            msg = (
                ...
                "Attribute fingerprint:  [{a}]".format(
                    c=calculated_fprint, a=self.fprint
                )
            )
    ...
    def download_ssh(self):
        ...
        with tempfile.NamedTemporaryFile(
            dir=_auth_keys_dir, delete=False
        ) as f:
            f.write(res)
...
def main(*args):
    ...
    parser.add_argument(
        '--engine-https-port',
        help="Define engine https port." " If not provided, will be used 443",
    )
    ...
    reg = Register(
        engine_fqdn=parsed_args.engine_fqdn,
