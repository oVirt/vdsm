...
def _getLibvirtConnStubEmpty():
    class ConnStub:
        def getCapabilities(self):
            return "<capabilities><host><cpu></cpu></host></capabilities>"
...
def _getLibvirtConnStubForTscScaling(scaling):
    class ConnStub:
        def getCapabilities(self):
            return (
                "<capabilities><host><cpu>"
                "<counter name='tsc' frequency='1234000000' scaling='" + scaling + "'/>"
                "</cpu></host></capabilities>"
            )
...
class TestCaps(TestCaseBase):
    ...
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_ibm_S822L.out"),
    )
    ...
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_s390x.out"),
    )
    @MonkeyPatch(numa, "memory_by_cell", lambda x: {"total": "1", "free": "1"})
    ...
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_amd_6274.out"),
    )
    @MonkeyPatch(numa, "memory_by_cell", lambda x: {"total": "1", "free": "1"})
    @MonkeyPatch(platform, "machine", lambda: cpuarch.X86_64)
    ...
    def testEmulatedMachines(self):
        ...
        machines = set(machinetype.emulated_machines(cpuarch.X86_64, capsData))
        expectedMachines = {
            "pc-1.0",
            "pc",
            "isapc",
    ...
    def test_parse_node_version(self):
        ...
        expected_results = (("", ""), ("1", ""), ("", "2"), ("1", "2"), ("", "2"))
        for test_input, expected_result in zip(inputs, expected_results):
            with tempfile.NamedTemporaryFile() as f:
                f.write(test_input)
                f.flush()
                self.assertEqual(osinfo._parse_node_version(f.name), expected_result)

    @MonkeyPatch(numa, "memory_by_cell", lambda x: {"total": "49141", "free": "46783"})
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_amd_6274.out"),
    ...
    def testNumaTopology(self):
        ...
        expectedNumaInfo = {
            "0": {
                ...
                "totalMemory": "49141",
                "hugepages": {4: {"totalPages": "2500"}, 2048: {"totalPages": "100"}},
            },
            "1": {
                "cpus": [8, 9, 10, 11, 12, 13, 14, 15],
                "totalMemory": "49141",
                "hugepages": {4: {"totalPages": "2"}, 2048: {"totalPages": "1"}},
            },
            "2": {
                "cpus": [16, 17, 18, 19, 20, 21, 22, 23],
                "totalMemory": "49141",
                "hugepages": {4: {"totalPages": "0"}, 2048: {"totalPages": "0"}},
            },
            "3": {
    ...
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_ibm_S822L_le.out"),
    )
    @MonkeyPatch(numa, "memory_by_cell", lambda x: {"total": "1", "free": "1"})
    ...
    @MonkeyPatch(
        libvirtconnection,
        "get",
        lambda: _getLibvirtConnStubFromFile("caps_libvirt_intel_i73770_nosnap.out"),
    )
    @MonkeyPatch(numa, "memory_by_cell", lambda x: {"total": "1", "free": "1"})
    ...
    @MonkeyPatch(
        commands, "run", lambda x: (_ for _ in ()).throw(Exception("A problem"))
    )
    def test_getFipsEnabledOffWhenError(self):
        self.assertFalse(caps._getFipsEnabled())

    @MonkeyPatch(
        libvirtconnection, "get", lambda: _getLibvirtConnStubForTscScaling("yes")
    )
    ...
    @MonkeyPatch(
        libvirtconnection, "get", lambda: _getLibvirtConnStubForTscScaling("no")
    )
    ...
    @MonkeyPatch(
        machinetype, "cpu_features", lambda: ["flag_3", "feature_1", "feature_2"]
    )
    ...
    def test_getNvmeHostNqn_not_found(self):
        self.assertEqual(caps._getNvmeHostNqn(), "")
