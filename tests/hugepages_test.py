    ...
    def test_supported(self):
        with namedTemporaryDir() as src:
            ...
            sizes = [
                ...
                for f in [
                    tempfile.NamedTemporaryFile(dir=src, delete=False)
                    for _ in range(3)
                ]
            ]
            with mock.patch(
                '{}.open'.format(hugepages.__name__),
...
class TestIntelligentAllocation(TestCaseBase):
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {2048: {'nr_hugepages': 12, 'free_hugepages': 12}},
    )
    ...
    def test_allocation_1_page(self):
        ...
        self.assertEqual(
            hugepages.calculate_required_allocation(
                cif, vm_hugepages, vm_hugepagesz
            ),
            1,
        )
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {2048: {'nr_hugepages': 8, 'free_hugepages': 4}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {2048: {'nr_hugepages': 16, 'free_hugepages': 4}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {2048: {'nr_hugepages': 4, 'free_hugepages': 4}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
...
class TestIntelligentDeallocation(TestCaseBase):
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {1048576: {'nr_hugepages': 17, 'free_hugepages': 0}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
    @MonkeyPatch(
        osinfo,
        'kernel_args_dict',
        lambda: {'hugepagesz': '1G', 'hugepages': '16'},
    )
    def test_deallocation_1_page(self):
        ...
        self.assertEqual(
            hugepages.calculate_required_deallocation(
                vm_hugepages, vm_hugepagesz
            ),
            1,
        )
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {1048576: {'nr_hugepages': 20, 'free_hugepages': 0}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
    @MonkeyPatch(
        osinfo,
    ...
    @MonkeyPatch(
        hugepages,
        'state',
        lambda: {1048576: {'nr_hugepages': 16, 'free_hugepages': 16}},
    )
    @MonkeyPatch(cpuarch, 'real', lambda: cpuarch.X86_64)
    @MonkeyPatch(
        osinfo,
