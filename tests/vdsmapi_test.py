...
from vdsm.api.schema_inconsistency_formatter import (
    SchemaInconsistencyFormatter,
)
...
_schema = vdsmapi.Schema.vdsm_api(
...
class FakeSchema(object):
    ...
    def with_types(types_yaml, arguments_yaml):
        ...
        schema_yaml = (
            types_header + types_yaml + method_header + arguments_yaml
        )
    ...
    def _schema_from(yaml_str):
        ...
        with mock.patch(
            '{}.io.open'.format(vdsmapi.__name__), mocked_open, create=True
        ):
...
class DataVerificationTests(TestCaseBase):
    ...
    def test_optional_params(self):
        params = {
            u"addr": u"rack05-pdu01-lab4.tlv.redhat.com",
            u"port": 54321,
            u"agent": u"apc_snmp",
            u"username": u"emesika",
            u"password": u"pass",
            u"action": u"off",
            u"options": u"port=15",
        }
    ...
    def test_unknown_param(self):
        params = {
            u"storagepoolID": u"00000002-0002-0002-0002-0000000000f6",
            u"onlyForce": True,
    ...
    def test_get_caps(self):
        ret = {
            'HBAInventory': {
                'iSCSI': [{'InitiatorName': 'iqn.1994-05.co'}],
                'FC': [],
            },
            'packages2': {
                'kernel': {'release': '201.fc23.x86_64', 'version': '4.5.5'},
                'glusterfs-rdma': {'release': '1.fc23', 'version': '3.7.11'},
                'glusterfs-fuse': {'release': '1.fc23', 'version': '3.7.11'},
                'spice-server': {'release': '1.fc23', 'version': '0.12.6'},
                'librbd1': {'release': '2.fc23', 'version': '0.94.7'},
                'vdsm': {
                    'release': '73.git2105bb3.fc23',
                    'version': '4.18.999',
                },
                'qemu-kvm': {'release': '10.fc23', 'version': '2.4.1'},
                'glusterfs': {'release': '1.fc23', 'version': '3.7.11'},
                'libvirt': {'release': '1.fc23', 'version': '1.2.18.3'},
                'qemu-img': {'release': '10.fc23', 'version': '2.4.1'},
                'mom': {'release': '1.fc23', 'version': '0.5.4'},
                'glusterfs-geo-replication': {
                    'release': '1.fc23',
                    'version': '3.7.1',
                },
                'glusterfs-server': {'release': '1.fc23', 'version': '3.7.11'},
                'glusterfs-cli': {'release': '1.fc23', 'version': '3.7.11'},
            },
    ...
    def test_create_complex_params(self):
        complex_type = {
            ...
            'metadata': {'LeaseMetadata': [['JobMetadata', 'object']]},
        }
    ...
    def test_no_params(self):
        self.assertEqual(
            _schema.get_args_dict('Host', 'getCapabilities'), None
        )

    def test_single_param(self):
        complex_type = {'vmID': {'UUID': 'UUID'}}
        self.assertEqual(
            _schema.get_args_dict('VM', 'getStats'),
            json.dumps(complex_type, indent=4),
        )
...
class SchemaTypeTest(TestCaseBase):
    ...
    @mock.patch.object(
        vdsmapi.SchemaType, "schema_dirs", return_value=("/a/b/c",)
    )
    ...
    def test_path_should_raise_in(self, exists_mock, schema_dirs_mock):
        expected_msg = (
            "Unable to find API schema file, tried: " "/a/b/c/vdsm-api.pickle"
        )
    ...
    def test_path_should_give_existing_path_in(
        self, exists_mock, schema_dirs_mock
    ):
...
class MethodArgumentsParsingTests(TestCaseBase):
    ...
    def test_nested_object_type(self):
        self.check_with_own_types(
            ...
            {
                "object_argument": {
                    "nested_object": {"some_int": "int", "some_bool": "bool"}
                }
            },
        )
...
class SchemaInconsistencyFormatterTest(TestCaseBase):
    ...
    def test_with_message_only(self):
        ...
        self.assertIn(
            u"Following parameters ['invalid'] were not recognized",
            log_entries,
        )
    ...
    def test_complex_type_visitor(self):
        ...
        self.assertIn(FakeSchema.METHOD_NAME, log_entries)
        self.assertIn(
            u'Provided value "42" not defined in MyEnum enum', log_entries
        )
        self.assertIn(u"With backtrace:", log_entries)
