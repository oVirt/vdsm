...
class TestVerifyUntrustedVolume(object):
    ...
    @pytest.mark.parametrize(
        'hsm_compat,config_compat,sd_version',
        [
            ('0.10', '0.10', 4),
            ('1.1', '0.10', 4),
            ('0.10', '1.1', 4),
    ...
    def test_valid_qcow2_compat(self, hsm_compat, config_compat, sd_version):
        with self.fake_volume(
            vol_fmt=sc.COW_FORMAT, sd_version=sd_version
        ) as vol:
    ...
    def test_disabled_compat_raises(
        self, hsm_compat, config_compat, sd_version
    ):
        with self.fake_volume(
            vol_fmt=sc.COW_FORMAT, sd_version=sd_version
        ) as vol:
    ...
    def fake_volume(self, vol_fmt, sd_version=3):
        with fake_file_env(sd_version=sd_version) as env:
            ...
            vol_id = make_uuid()
            make_file_volume(
                env.sd_manifest, self.SIZE, img_id, vol_id, vol_format=vol_fmt
            )
