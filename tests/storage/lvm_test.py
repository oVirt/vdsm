...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_pv_stale_reload_one_stub(stale_pv, read_only):
...
def test_pv_stale_reload_all_stub(stale_pv, read_only):
    ...
    assert set(lvm.getAllPVs()) == {
        lvm.Unreadable(good_pv_name, True),
        lvm.Unreadable(stale_pv_name, True)
    }
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_pv_stale_reload_all_clear(stale_pv, read_only):
    vg_name, good_pv_name, stale_pv_name = stale_pv
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_vg_invalidate(tmp_storage, read_only):
    dev_size = 1 * 1024**3
    ...
    lvm.createLV(vg1_name, "lv1", 128, activate=False, read_only=False)

    lvm.createVG(vg2_name, [dev2], "initial-tag", 128)
    lvm.createLV(vg2_name, "lv2", 128, activate=False, read_only=False)
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_vg_invalidate_lvs(tmp_storage, read_only):
    dev_size = 1 * 1024**3
    dev = tmp_storage.create_device(dev_size)
    vg_name = str(uuid.uuid4())
    ...
    lvm.createVG(vg_name, [dev], "initial-tag", 128)
    lvm.createLV(vg_name, "lv1", 128, activate=False, read_only=False)
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_vg_stale_reload_one_stub(stale_vg, read_only):
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_vg_stale_reload_one_clear(stale_vg, read_only):
    good_vg_name, stale_vg_name = stale_vg
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_vg_stale_reload_all_clear(stale_vg, read_only):
    good_vg_name, stale_vg_name = stale_vg
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_lv_stale_reload_one_stub(stale_lv, read_only):
    vg_name, good_lv_name, stale_lv_name = stale_lv
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_lv_stale_reload_one_clear(stale_lv, read_only):
    vg_name, good_lv_name, stale_lv_name = stale_lv
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_lv_stale_reload_all_stub(stale_lv, read_only):
    vg_name, good_lv_name, stale_lv_name = stale_lv
...
@pytest.mark.root
@pytest.mark.parametrize("read_only", [True, False])
def test_lv_stale_reload_all_clear(stale_lv, read_only):
    vg_name, good_lv_name, stale_lv_name = stale_lv
...
def test_lv_reload_error_one_stub(fake_devices, fake_runner):
    fake_runner.rc = 5
    fake_runner.err = [b"Fake lvm error"]
    lc = lvm.LVMCache()
    lc._lvs = {
