...
class v2vTests(TestCaseBase):
    ...
    def testV2VOutput(self):
        cmd = [FAKE_VIRT_V2V.cmd,
               ...
               '-of', 'raw',
               '-oa', 'sparse',
               '-oo', 'vdsm-image-uuid=' + self.image_id_a,
               '-oo', 'vdsm-vol-uuid=' + self.volume_id_a,
               '-oo', 'vdsm-image-uuid=' + self.image_id_b,
               '-oo', 'vdsm-vol-uuid=' + self.volume_id_b,
               '--password-file', '/tmp/mypass',
               '-oo', 'vdsm-vm-uuid=' + self.job_id,
               '-oo', 'vdsm-ovf-output=/usr/local/var/run/vdsm/v2v',
               '--machine-readable',
