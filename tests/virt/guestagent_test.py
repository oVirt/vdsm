...
_INPUTS = [
    ...
    {'version': '2.6.32-71.el6.x86_64'},
    {
        'interfaces': [
            {
                'hw': '00:21:cc:68:d7:38',
                'name': 'eth0',
                'inet': ['9.115.122.77'],
                'inet6': ['fe80::221:ccff:fe68:d738'],
            },
            {
                'hw': 'a0:88:b4:f0:ce:a0',
                'name': 'wlan0',
                'inet': ['9.115.126.23'],
                'inet6': ['fe80::a288:b4ff:fef0:cea0'],
            },
            {
                'hw': '52:54:00:5b:3f:e1',
                'name': 'virbr0',
                'inet': ['192.168.122.1'],
                'inet6': [],
            },
    ...
    {'applications': ['kernel-2.6.32-71.7.1.el6', 'kernel-2.6.32-220.el6']},
    {
        'disks': [
            {
                'total': 130062397440,
...
class TestFiltering(TestCaseBase):
    ...
    def test_filter_xml_chars_timing(self):
        setup = (
            'from vdsm.virt.guestagent import _filterXmlChars;'
            'x = u"x" * 1024 * 1024'
        )
...
class TestGuestIF(TestCaseBase):
    ...
    def test_handleMessage(self):
        fakeGuestAgent = guestagent.GuestAgent(
            None, None, self.log, lambda: None, lambda: None
        )
    ...
    def test_guestinfo_encapsulation(self):
        fake_guest_agent = guestagent.GuestAgent(
            None, None, self.log, lambda: None, lambda: None
        )
...
class TestGuestIFHandleData(TestCaseBase):
    ...
    def setUp(self):
        self.fakeGuestAgent = guestagent.GuestAgent(
            None, None, self.log, lambda: None, lambda: None
        )
    ...
    def testMixed(self):
        ...
        for t in zip(_MSG_TYPES, _INPUTS, _OUTPUTS):
            ...
            for chunk in self.messageChunks(msgStr, self.maxMessageSize):
                ...
                if chunk[-1] != '\n':
                    assert (
                        self.fakeGuestAgent._messageState
            ...
            for k, v in t.assertDict.items():
                if isOverLimit:
                    ...
                    assert (
                        self.fakeGuestAgent.guestInfo[k]
                        == self.infoDefaults[k]
                    )
                else:
...
class DiskMappingTests(TestCaseBase):
    ...
    def setUp(self):
        self.agent = guestagent.GuestAgent(
            None, None, None, lambda: None, lambda: None
        )
