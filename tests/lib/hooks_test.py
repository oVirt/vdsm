...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        pytest.param(
            [DirEntry("nested", 0o777, [FileEntry("executable", 0o777, "")])],
...
@pytest.mark.parametrize(
    "data, expected",
    [
        pytest.param(None, u"", id="no XML data"),
        pytest.param(u"", u"", id="empty XML data"),
        pytest.param(
            u"<abc>def</abc>", u"<abc>def</abc>", id="simple XML data"
        ),
    ],
)
def test_rhd_should_handle_xml_data(dummy_hook, hooks_dir, data, expected):
    result = hooks._runHooksDir(
        data, hooks_dir.basename, hookType=hooks._DOMXML_HOOK
    )
...
@pytest.mark.parametrize(
    "data, expected",
    [
        pytest.param(None, None, id="no JSON data"),
        pytest.param({"abc": "def"}, {"abc": "def"}, id="simple JSON data"),
        pytest.param(
...
def test_rhd_should_handle_json_data(dummy_hook, hooks_dir, data, expected):
    result = hooks._runHooksDir(
        data, hooks_dir.basename, hookType=hooks._JSON_HOOK
    )
    assert result == expected
...
@pytest.mark.parametrize(
    ...
    indirect=True,
    argvalues=[
        pytest.param(perm, id="-".join(script.name for script in perm))
...
def test_rhd_should_report_invalid_hook_error_codes(caplog, hooks_dir):
    ...
    assert "111" in "".join(
        msg for (_, lvl, msg) in caplog.record_tuples if lvl == logging.WARNING
    )
...
def test_rhd_should_report_hook_stderr(caplog, hooks_dir):
    ...
    assert "1.sh" in "".join(
        msg for _, lvl, msg in caplog.record_tuples if lvl == logging.INFO
    )
...
@pytest.mark.parametrize(
    "vmconf, params, expected",
    [
        pytest.param({}, {"abc": "def"}, {"abc": "def"}, id="simple variable"),
        pytest.param(
            {},
            ...
            marks=pytest.mark.xfail(
                on_ascii_locale(), reason="no support for localized chars"
            ),
        ),
        pytest.param(
            {},
        ...
        pytest.param({"vmId": "myvm"}, {}, {"vmId": "myvm"}, id="VM id"),
        pytest.param(
            {"custom": {"abc": "def"}}, {}, {"abc": "def"}, id="vmconf param"
        ),
        pytest.param(
            {"custom": {"abc": "geh"}},
            {"abc": "def"},
...
def test_rhd_should_assemble_environment_for_hooks(
    hooks_dir, env_dump, vmconf, params, expected
):
...
@pytest.mark.parametrize(
    "var_name, hook_type",
    [
        pytest.param("_hook_domxml", hooks._DOMXML_HOOK, id="xml hook"),
        pytest.param("_hook_json", hooks._JSON_HOOK, id="JSON hook"),
    ],
)
def test_rhd_should_pass_data_file_to_hooks(
    hooks_dir, env_dump, mkstemp_path, var_name, hook_type
):
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        pytest.param([], "", id="non-existent script"),
    ],
)
...
@pytest.mark.parametrize(
    "flag",
    [
        pytest.param(libvirt.VIR_DOMAIN_NONE, id="libvirt.VIR_DOMAIN_NONE"),
        pytest.param(
