...
class TestIpAddressHostTail(VdsmTestCase):
    ...
    def test_hosttail_ipv4(self):
        self.assertEqual(
            ('1.2.3.4', '4321'), ipaddress.hosttail_split('1.2.3.4:4321')
        )

    def test_hosttail_ipv6(self):
        self.assertEqual(
            ('2001::1', '4321'), ipaddress.hosttail_split('[2001::1]:4321')
        )

    def test_hosttail_namedhost(self):
        self.assertEqual(
            ('TestHost', '4321'), ipaddress.hosttail_split('TestHost:4321')
        )

    def test_hosttail_hostpath(self):
        self.assertEqual(
            ('FQDN.host', '/ovirt/rules/the/world'),
            ipaddress.hosttail_split('FQDN.host:/ovirt/rules/the/world'),
        )

    def test_hosttail_hostpath_with_colon_in_path(self):
        self.assertEqual(
            ('FQDN.host', '/path/a:b:c'),
            ipaddress.hosttail_split('FQDN.host:/path/a:b:c'),
        )
    ...
    def test_literal_ipv4_addr(self):
        self.assertEqual(
            '1.2.3.4', ipaddress.normalize_literal_addr('1.2.3.4')
        )

    def test_literal_ipv6_addr(self):
        self.assertEqual(
            '[2001::1]', ipaddress.normalize_literal_addr('2001::1')
        )

    def test_literal_namedhost(self):
        self.assertEqual(
            'namedhost', ipaddress.normalize_literal_addr('namedhost')
        )
